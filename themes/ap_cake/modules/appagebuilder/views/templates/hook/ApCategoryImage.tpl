	...
	<li class="cate_{$category.id_category|intval}">
		<a href="{$link->getCategoryLink($category.id_category, $category.link_rewrite)|escape:'html':'UTF-8'}">
			<span class="cate_content">
				...
				<span class="{if $level == 0}cate-parent {/if}cate-name">{$category.name|escape:'html':'UTF-8'}</span>
			</span>
		</a>
			...
			<span class="cate_content">
				...
				{if $formAtts.cate_depth == 1}
					<span class="nbproducts-child">
						<span class="name-cat">{$category.name|escape:'html':'UTF-8'}</span>
						<span id="leo-cat-{$category.id_category}" class="leo-qty" data-str="{l s='products' mod='appagebuilder'}"></span>
					</span>
				{else}
					<span class="name-cat">{$category.name|escape:'html':'UTF-8'}</span>
				{/if}
